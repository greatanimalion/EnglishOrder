...
import { getHomeData, getPublicData } from '@/services/home'
import a from "@/assets/img/1.jpg"
import { useEffect, useState } from 'react';

const Title = () => {
  const [publicDate, setPublicDate] = useState<any[]>([])
  useEffect(() => {
    getPublicData().then(res => {
      setPublicDate(res.data.list)
    })
  }, [])
  return <>
    {
      publicDate.length==0?<List></List>:publicDate.map((item) => (
        <div className={style.ellipsis} key={item.id}>
              ...
              <div style={{ width: '100%' }}>
                ...
                <div style={{ margin: '10px 0' }} >
                  ...
                  <div className={style.more}>
                    ...
                    <div>{item.collect}收藏</div>
                    <div>{item.read}阅读量</div>
                  </div>
                </div>
              </div>
            </div>
